...
void FighterInstance::RerollName(Amount cost, const RoConfig& cfg,  xaya::Random& rnd, pxd::Quality ql)
{
	...
	/* Default to the top bracket so a payment at or above the highest price maps
	   to the maximum probability bonus.  With init 0 the search loop (which only
	   assigns on `cost < price[i]`) never reassigned for an at/above-top payment,
	   so the biggest payers wrongly got lookUpProbability[0] (the LOWEST bonus). */
	int32_t probabilityIndex = (int32_t) lookUpPrices.size() - 1;
