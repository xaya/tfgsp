...
{
	  // Original game code had wrong resource distribution weights,
	  // for now lets use the graphs to smooth distribution out
    ...
    for(int roll = 0; roll < rollCount; ++roll)
    {
        ...
        for(auto& rw: rewardTableDb.rewards())
        {
			...
			if(rolCurNumC <= accumulatedWeight)
			{
			  ...
			  {
				  if(blueprintAuthID != "00000000-0000-0000-zzzz-zzzzzzzzzzzz" && blueprintAuthID != "c064e7f7-acbf-4f74-fab8-cccd7b2d4004")
				  {
          ...
          if(tnm->GetInstance().blocksleft() == 0)
          {
			...
			{"dd89aedb-06c1-1964-d90b-b5977ee61c4c", fpm::fixed_24_8(4246)},
              ...
              for(const auto& participant: participatingPlayerTotalScore)
              {
				  ...
				  for(auto& rw: rewardTableDb.rewards())
				  {
					 totalWeight = totalWeight + fpm::fixed_24_8(rw.weight()) * m[tnm->GetProto().authoredid()];
				  }
                  ...
                  for(uint32_t roll = 0; roll < rollCount; ++roll)
                  {
                      ...
                      for(auto& rw: rewardTableDb.rewards())
                      {
						  ...
						  {					  
							  if((pxd::RewardType)(int)rw.type() == pxd::RewardType::CraftedRecipe || (pxd::RewardType)(int)rw.type() == pxd::RewardType::GeneratedRecipe)
							  {
								  if (rnd.NextInt((int)r[tnm->GetProto().authoredid()]) > 1000)
								  {
									   break;
								  } 
							  }  
