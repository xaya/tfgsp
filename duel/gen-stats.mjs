...
// top-level "tunables" block and a top-level "_readme" block — the design rules the
// table is authored against: what `power` means for each of the nine effects, the
// trades-damage-for-utility rule, and the flat-throughput ladders. READ IT before
// touching a number; `duel/test/duel_tests --balance-report` is the evidence tool
// that checks the table still obeys it). This script:
...
// `_readme` is the authoring rules (JSON has no comments, so the header comment has to
// BE a key) and `tunables` the engine constants -- neither is a move, so both are split
// off before the GUID-set cross-check below, which treats every remaining key as a move
// AuthoredId and fails loudly on any that isn't.
const { tunables, _readme, ...statsByGuid } = raw;
if (!tunables) fail(`${STATS_JSON} missing top-level "tunables"`);
if (!Array.isArray(_readme) || _readme.length === 0) {
  fail(`${STATS_JSON} missing top-level "_readme" (the authoring rules -- see the balance pass)`);
}
...
// ---- 4c. emit duel/test/moves_meta_gen.h (TEST-BINARY ONLY) ----
//
// Name + quality per dense index. These live HERE and not in stats_gen.h on purpose:
// stats_gen.h is compiled into the wasm reactor, and 39 name strings would be 39 strings
// of dead weight in a blob whose whole selling point is that it has no imports and no
// slack. Nothing but duel/test/test_main.cpp's `--balance-report` reads this file, so the
// shipped native/wasm engines stay byte-identical to what they were without it.
const metaRows = entries
  .map((e) => `    {${JSON.stringify(e.name)}, ${e.quality}}, // [${e.idx}]`)
  .join('\n');
const meta = `// duel/test/moves_meta_gen.h — GENERATED by duel/gen-stats.mjs — do not edit.
//
...
`;
const OUT_META = join(HERE, 'test', 'moves_meta_gen.h');
...
export interface DuelMoveStat {
  ...
  quality: number;
  /**
   * What \`power\` means depends on the effect (audited against the engine's resolve loop;
...
console.log(`gen-stats.mjs: wrote ${OUT_META}`);
