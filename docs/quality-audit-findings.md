...
| FN6 | B | src/logic.cpp:547 | dead-code | low | neutral | Unused local `rating` in ResolveExpedition | TODO |
...
| FN18 | B | src/moveprocessor.cpp:1466-1477 | dry-duplication | low | neutral | Same O(n^2) dedup-and-reject pattern copy-pasted 5 times | TODO |
| FN19 | B | src/moveprocessor.cpp:1116 | dead-code | low | neutral | Dead `const std::string& name` parameter duplicating a.GetName() in many Parse* helpers | TODO |
| FN20 | B | src/moveprocessor.cpp:587-607 | overengineering | low | neutral | Redundant `const Context& ctx` parameter shadowing the member in GetRecepieObjectFromID/GetCandyKeyNameFromID | TODO |
| FN21 | B | src/moveprocessor.cpp:600-602 | dead-code | low | neutral | Unreachable `break;` after `return candy.first;` | TODO |
...
| FN32 | B | src/moveprocessor.cpp:3513-3520 | clarity | low | neutral | 296-element name-price lookup table inlined and rebuilt every call | TODO |
| FN33 | B | src/params.hpp:36-56 | dead-code | medium | neutral | Entire Params class is dead (no methods, never read) | TODO |
...
| FN35 | B | src/jsonutils.cpp:45-53 | dead-code | medium | neutral | CoinAmountFromJson declared/defined but never called | TODO |
| FN36 | B | src/jsonutils.cpp:33-40 | taurion-baggage | low | neutral | Stale vCHI comment on coin/id constants | TODO |
...
| FN50 | B | database/inventory.cpp:55-117 | dead-code | medium | neutral | QuantityProduct (GMP bignum helper) is dead outside tests; pulls in the whole GMP dependency | TODO |
...
| FN60 | B | database/fighter.cpp:110-131 | overengineering | low | neutral | Animations are re-collected and re-sorted inside the per-move loop in FighterInstance constructor | TODO |
| FN61 | B | database/recipe.cpp:87-138 | dry-duplication | low | neutral | RecipeInstance proto constructor takes CraftedRecipe by value and duplicates the field-copy of the string-GUID constructor | TODO |
