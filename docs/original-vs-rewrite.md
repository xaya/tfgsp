...
| DOS-FC | tournament entry | roster array `fc` (`ParseTournamentEntryData`, `moveprocessor_activity.cpp`) was the lone user-supplied id-array with no `MAX_MOVE_ARRAY` cap — the O(n²) `HasDuplicates` dedup + per-entry `GetById` lookups ran before the `size()!=teamsize()` reject **and** before the joincost payment gate, so one cheap move carrying a huge `fc` forces super-linear per-block work on every honest node | DoS (deterministic per-block near-halt; **not** a fork — work is byte-identical on all nodes) | DONE (`0bdab5e`, `2026-07-01`) — cap after the `isArray()` check mirroring the expedition `fid` sibling; Pass-B regression test (`OversizedMoveArraysAreCappedNotFatal`) extended with a huge-`fc` entry move; golden byte-identical, full `make check` + determinism lint green. Surfaced by the launch sign-off re-verification audit |
| Faction/PlayerRole | accounts | Taurion `Faction` had been refactored into an inert `PlayerRole` enum (nothing set a role, nothing gated on it — god-mode uses the roconfig `god_mode()` bool; sole assignment was a hardcoded `name=="tftr"` block the author flagged "leftover from original source"). It was still consensus state: a `role` DB column (always NULL) + a `"role"` account-JSON field (always `"i"`) | Taurion-baggage removal (consensus — golden regen; client `role` JSON field dropped) | DONE (`2026-07-01`, owner-approved) — removed the enum, `ResultWithRole`, the `role` column, the JSON field, all Bind/ToString/FromString/GetNullable + Set/GetRole + `QueryInitialised` + the now-empty `xayaplayer.tpp` + every role test ref; reworded 6 residual "faction" comments. Golden **regenerated**; full `make check` + determinism lint green |
