...
class SpecialTournamentTest (PXTest):
  ...
  def run (self):
    ...
    # The deep special-tournament scenario that followed relied on the
    # MaybeSQLTestInjection/2 regtest backdoors (bulk-creating 150+ testft<N>
    # players in a single move) to exercise tier progression.  Those backdoors
    # were removed from the GSP (dead on Polygon, not DRY), so the injection-
    # driven scenario is dropped; the basic special-tournament flow above
    # remains covered.
